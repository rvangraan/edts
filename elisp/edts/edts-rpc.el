      ...
      (with-current-buffer buffer
        (let* ((proc (get-buffer-process (current-buffer)))
          ...
          (if (not retry)
            ...
            (edts-rpc-request command args body t)))))))
...
(defun edts-rpc--log-http-response (url)
    (edts-log-debug "Reply %s received for request to %s"
                    (edts-rpc--parse-http-response-status)
                    url)
    (edts-log-debug-2 "Reply:\n----\n%s\n----" (buffer-string)))
...
;; (defun edts-rpc-encode-args (args)
;;   "Encode ARGS as a list of url-arguments."
;;   (let ((encoded "?")
;;         (arg nil))
;;     (while args
;;       (setq arg (edts-rpc-encode-arg (pop args)))
...
(when (featurep 'ert)
  ...
  (ert-deftest edts-rpc-encode-arg-test ()
    (should (equal "foo=bar"
            (edts-rpc-encode-arg '("foo" . "bar"))))
    (should (equal "foo=bar"
            (edts-rpc-encode-arg '("foo" . ("bar")))))
    (should (equal "foo=bar,baz"
            (edts-rpc-encode-arg '("foo" . ("bar" "baz")))))))
