...
do_handle_request(Req) ->
  P = Req:get(path),
  io:fwrite(user, <<"~p ~p ~p: P = ~p~n">>, [self(), ?MODULE, ?LINE, P]),
  case [list_to_atom(E) || E <- string:tokens(Req:get(path), "/")] of
    [Command] ->
      io:fwrite(user, <<"~p ~p ~p~n">>, [self(), ?MODULE, ?LINE]),
      C = get_input_context(Req),
      io:fwrite(user, <<"~p ~p ~p: C = ~p~n">>, [self(), ?MODULE, ?LINE, C]),
      A = edts_cmd:run(Command, C),
