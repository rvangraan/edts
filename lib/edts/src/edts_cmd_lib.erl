...
validate(Ctx0, Keys) ->
  ...
  try
    ...
    throw:{error, Key0} = E ->
      ...
      Value = orddict:fetch(atom_to_list(Key), Ctx0),
...
modules_validate(Ctx) ->
  case orddict:find(modules, Ctx) of
    error       -> {ok, all};
    {ok, "all"} -> {ok, all};
    {ok, Val}   -> {ok, [list_to_atom(Mod) || Mod <- string:tokens(Val, ",")]}
  end.
...
nodename_validate(Ctx) ->
  {ok, edts_util:make_nodename(orddict:fetch(node_name, Ctx))}.
